...
async function openAppleCalendar(session, athleteName) {
...
async function openAllAppleCalendar(sessions, athleteName) {
  ...
  const file = new File([ics], 'courtflow-sessions.ics', { type: 'text/calendar' })
  if (typeof navigator.share === 'function' && navigator.canShare?.({ files: [file] })) {
    try { await navigator.share({ files: [file] }) } catch { /* user cancelled */ }
    return
  }
